...
//    [self setCustomPageControlDotImage:currentPageDotImage isCurrentPageDot:YES];
    if (self.pageControlStyle != SDCycleScrollViewPageContolStyleLineSD) {
        self.pageControlStyle = SDCycleScrollViewPageContolStyleLineSD;
    }
}
...
- (void)setCustomPageControlDotSDImage:(UIImage *)image isCurrentPageDot:(BOOL)isCurrentPageDot
{
    if (!image || !self.pageControl) return;
    
//    if ([self.pageControl isKindOfClass:[SSLPageControl class]]) {
//        TAPageControl *pageControl = (TAPageControl *)_pageControl;
//
//        dragonPageControl.numberOfPages = self.imagePathsGroup.count;
//        dragonPageControl.center = CGPointMake(self.frame.size.width / 2, self.frame.size.height - 70);
//        dragonPageControl.pageIndicatorTintColor   = [UIColor blueColor];
//        dragonPageControl.defersCurrentPageDisplay = YES;
//        [self addSubview:dragonPageControl];
//        _pageControl = (UIControl *)dragonPageControl;
//    }
}
...
- (void)setupPageControl
{
    ...
    switch (self.pageControlStyle) {
            case SDCycleScrollViewPageContolStyleLineSD:
        {
            SSLPageControl * dragonPageControl = [[SSLPageControl alloc] initWithStyel:SSLPageControlStyelMixRectanglePoint andCount:self.imagePathsGroup.count];
            dragonPageControl.numberOfPages = self.imagePathsGroup.count;
            dragonPageControl.center = CGPointMake(self.frame.size.width / 2, self.frame.size.height - 70);
            dragonPageControl.pageIndicatorTintColor   = LightPurpleBtnColor;
            dragonPageControl.currentPageIndicatorTintColor = [UIColor whiteColor];
//            dragonPageControl.pageImage = self.pageDotImage;//未选中
//            dragonPageControl.currentPageImage = self.currentPageDotImage;
            dragonPageControl.defersCurrentPageDisplay = YES;
            [self addSubview:dragonPageControl];
            _pageControl = (UIControl *)dragonPageControl;
        }
